...
DB_login = 'C:/Users/82104/Desktop/db_login.txt'
f = open(DB_login, 'r')
lines = f.readlines()
f.close()
...
def sell_process(revenue=1.01):
    ...
    for x in check_list:
        ...
        if upbit.get_order(x)['state'] == 'done' and x not in done_list: 
            ...
            # DB에 저장
            json_data = upbit.get_order(x)
            sql = "insert into trade_result (created_at,executed_volume,locked_,market,ord_type,paid_fee,price,remaining_fee,remaining_volume,reserved_fee,side,state,trades_count,uuid,volume) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
            val = (json_data['created_at'],json_data['executed_volume'],json_data['locked'],json_data['market'],json_data['ord_type'],json_data['paid_fee'],json_data['price'],
                   json_data['remaining_fee'],json_data['remaining_volume'],json_data['reserved_fee'],json_data['side'],json_data['state'],json_data['trades_count'],json_data['uuid'],json_data['volume'])
    ...
    if (float(i['avg_buy_price'])*1.01+one_tick(float(i['avg_buy_price'])) <= current_price): #1.01+1틱을 설정한 이유: 보수적인 구매전략
        ...
        if 'error' in ret.keys():
            ...
            #DB에 저장
            json_data = upbit.get_order(ret['uuid'])
            sql = "insert into trade_result (created_at,executed_volume,locked_,market,ord_type,paid_fee,price,remaining_fee,remaining_volume,reserved_fee,side,state,trades_count,uuid,volume) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
            val = (json_data['created_at'],json_data['executed_volume'],json_data['locked'],json_data['market'],json_data['ord_type'],json_data['paid_fee'],json_data['price'],
                   json_data['remaining_fee'],json_data['remaining_volume'],json_data['reserved_fee'],json_data['side'],json_data['state'],json_data['trades_count'],json_data['uuid'],json_data['volume'])
            curs.execute(sql,val)
